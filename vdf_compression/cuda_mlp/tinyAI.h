...
template <typename T, BACKEND Backend = BACKEND::HOST,
          ACTIVATION Activation = ACTIVATION::TANH>
class NeuralNetwork {
public:
  NeuralNetwork(std::vector<int> &arch, GENERIC_TS_POOL::MemPool *pool,
                NumericMatrix::Matrix<T, Backend> &input,
                NumericMatrix::Matrix<T, Backend> &output, size_t batchSize,
                int seed = 42)
    ...
    inputData = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = input.ncols(), .rows = input.nrows()};
    outputData = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = output.ncols(), .rows = output.nrows()};
    input.getView(inputData, 0);
    output.getView(outputData, 0);
    sample = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = input.ncols(), .rows = batchSize};
    target = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = output.ncols(), .rows = batchSize};
    sample_t =
        NumericMatrix::Matrix<T, Backend>(input.ncols(), batchSize, _pool);
  ...
  void backward(const NumericMatrix::MatrixView<T> &sample,
                const NumericMatrix::MatrixView<T> &target) noexcept {
    spdlog::stopwatch timer;
    auto &curr_layer = layers.back();
    NumericMatrix::matsub(curr_layer.a, target, curr_layer.delta_store,
                          &handle);
    NumericMatrix::mat_pointwise_activate_prime<T, Activation>(
        curr_layer.z, curr_layer.a_prime,curr_layer.wmega);
    NumericMatrix::mat_pointwise_mul(curr_layer.delta_store, curr_layer.a_prime,
                                     curr_layer.delta);
    ...
    if (layers.size() == 1) {
      ...
      NumericMatrix::transpose_into(layers[layers.size() - 2].a,
                                    layers[layers.size() - 2].a_t);
      NumericMatrix::matmul(layers[layers.size() - 2].a_t, curr_layer.delta,
                            curr_layer.dw, &handle);
    }
    ...
    for (int i = layers.size() - 2; i >= 0; i--) {
      ...
      NumericMatrix::matmul(next_layer.delta, next_layer.w_t, next_layer.buffer,
                            &handle);
      NumericMatrix::mat_pointwise_activate_prime<T, Activation>(
          curr_layer.z, curr_layer.a_prime,curr_layer.wmega);
      NumericMatrix::mat_pointwise_mul(next_layer.buffer, curr_layer.a_prime,
                                       curr_layer.delta);
      // curr_layer.dw.zero_out();
      // curr_layer.db.zero_out();
      if (i == 0) {
        NumericMatrix::matmul(sample_t, curr_layer.delta, curr_layer.dw,
                              &handle);
      } else {
        NumericMatrix::transpose_into(layers[i - 1].a, layers[i - 1].a_t);
        NumericMatrix::matmul(layers[i - 1].a_t, curr_layer.delta,
                              curr_layer.dw, &handle);
      }
      NumericMatrix::matsum_rows(curr_layer.delta, curr_layer.db);
    }
  ...
  void update_weights() noexcept {
    ...
    for (std::size_t i = 0; i < layers.size(); i++) {
      ...
      T lr = 1e-1;
      NumericMatrix::matscale(current.dw, static_cast<T>(lr / batchSize_in_use),
                              &handle);
      NumericMatrix::matscale(current.db, static_cast<T>(lr / batchSize_in_use),
                              &handle);
  ...
  T train(std::size_t batchSize, T lr = 1e-3) noexcept {
    ...
    NumericMatrix::Matrix<T, Backend> error = NumericMatrix::Matrix<T, Backend>(
        target.nrows(), target.ncols(), _pool);
    ...
    if constexpr (Backend == BACKEND::DEVICE) {
      ...
      NumericMatrix::HostMatrix<T> shuffle_in(inputData.nrows(),
                                              inputData.ncols());
      NumericMatrix::HostMatrix<T> shuffle_out(outputData.nrows(),
                                               outputData.ncols());
      ...
      for (uint64_t i = numberOfInputRows - 1; i > 0; --i) {
        ...
        // Input
        std::memcpy(buffer_in, &shuffle_in(target, 0),
                    numberOfInputCols * sizeof(T));
        std::memcpy(&shuffle_in(target, 0), &shuffle_in(i, 0),
                    numberOfInputCols * sizeof(T));
        std::memcpy(&shuffle_in(i, 0), buffer_in,
                    numberOfInputCols * sizeof(T));
        // Output
        std::memcpy(buffer_out, &shuffle_out(target, 0),
                    numberOfOutputCols * sizeof(T));
        std::memcpy(&shuffle_out(target, 0), &shuffle_out(i, 0),
                    numberOfOutputCols * sizeof(T));
        std::memcpy(&shuffle_out(i, 0), buffer_out,
                    numberOfOutputCols * sizeof(T));
      }
      delete[] buffer_in;
      delete[] buffer_out;
    ...
    for (size_t i = 0; i < inputData.nrows(); i += batchSize) {
      ...
      if constexpr (Backend == BACKEND::HOST) {
        ...
        T tmp = 0.0;
        tinyAI_gpuMemcpy(&tmp, dev_loss, sizeof(T),
  ...
  void update_weights_adamw(size_t iteration, T lr, T beta1 = 0.9,
    ...
    for (auto &curr_layer : layers) {
      ...
      NumericMatrix::matscale(curr_layer.dw, static_cast<T>(1.0 - beta1),
                              &handle);
      NumericMatrix::matadd(curr_layer.m_w, curr_layer.dw, curr_layer.m_w,
                            &handle);

      NumericMatrix::matscale(curr_layer.v_w, beta2, &handle);
      NumericMatrix::mat_pointwise_mul(curr_layer.dw, curr_layer.dw,
                                       curr_layer.tmp);
      NumericMatrix::matscale(curr_layer.tmp, static_cast<T>(1.0 - beta2),
                              &handle);
      NumericMatrix::matadd(curr_layer.v_w, curr_layer.tmp, curr_layer.v_w,
                            &handle);

      T m_hat_scale = static_cast<T>(1.0) / (1 - std::pow(beta1, iteration));
      T v_hat_scale = static_cast<T>(1.0) / (1 - std::pow(beta2, iteration));

      NumericMatrix::matscale_to(curr_layer.m_w, curr_layer.mw_hat, m_hat_scale,
                                 &handle);
      NumericMatrix::matscale_to(curr_layer.v_w, curr_layer.vw_hat, v_hat_scale,
                                 &handle);
      NumericMatrix::mat_pointwise_sqrt(curr_layer.vw_hat, curr_layer.vw_hat);
      NumericMatrix::matadd_scalar(curr_layer.vw_hat, curr_layer.vw_hat,
                                   epsilon, &handle);

      NumericMatrix::matscale_to(curr_layer.m_w, curr_layer.w_decay, decay,
                                 &handle);

      NumericMatrix::mat_pointwise_div(curr_layer.mw_hat, curr_layer.vw_hat,
                                       curr_layer.tmp);
      NumericMatrix::matadd(curr_layer.tmp, curr_layer.w_decay, curr_layer.tmp,
                            &handle);
      NumericMatrix::matscale(curr_layer.tmp, lr, &handle);
      NumericMatrix::matsub(curr_layer.w, curr_layer.tmp, curr_layer.w,
                            &handle);
  ...
  std::tuple<T, T, std::vector<size_t>>
  quantize(const std::vector<T> &inputVector, int bits) {
  ...
  std::vector<T> dequantize(T delta, T minVal,
                            const std::vector<size_t> &kappa) {
...
private:
  void migrate_to_batchsize(std::size_t new_batchsize) {
    ...
    sample = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = inputData.ncols(), .rows = new_batchsize};
    target = NumericMatrix::MatrixView<T>{
        ._data = nullptr, .cols = outputData.ncols(), .rows = new_batchsize};
    sample_t = NumericMatrix::Matrix<T, Backend>(inputData.ncols(),
                                                 new_batchsize, _pool);
