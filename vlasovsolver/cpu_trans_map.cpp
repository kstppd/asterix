...
//    MPI_Comm_rank(MPI_COMM_WORLD,&myRank);
   ...
   switch(dimension) {
   case 0:
      if(direction > 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_X);
      if(direction < 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_X);
      break;
   case 1:
      if(direction > 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_Y);
      if(direction < 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_Y);
      break;
   case 2:
      if(direction > 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_Z);
      if(direction < 0) mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_Z);
      break;
   }
