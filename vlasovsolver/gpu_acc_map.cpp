...
__global__ void __launch_bounds__(WID3*WID3_PER_BLOCK/*, ACCELERATION_KERNEl_MIN_BLOCKS*/) acceleration_kernel(
   ...
   for (uint columnIndex = 0;
      ...
      for (int b = 0; b < nBlocks; b++) {
         ...
         Realf a[5];//
         {
            const Realf value0 = gpu_blockDataOrdered[valuesOffset+(k-4)*WID2+ij];
            const Realf value1 = gpu_blockDataOrdered[valuesOffset+(k-3)*WID2+ij];
            const Realf value2 = gpu_blockDataOrdered[valuesOffset+(k-2)*WID2+ij];
            const Realf value3 = gpu_blockDataOrdered[valuesOffset+(k-1)*WID2+ij];
            const Realf value4 = gpu_blockDataOrdered[valuesOffset+(k)*WID2+ij];
            const Realf value5 = gpu_blockDataOrdered[valuesOffset+(k+1)*WID2+ij];
            const Realf value6 = gpu_blockDataOrdered[valuesOffset+(k+2)*WID2+ij];
            const Realf value7 = gpu_blockDataOrdered[valuesOffset+(k+3)*WID2+ij];
            const Realf value8 = gpu_blockDataOrdered[valuesOffset+(k+4)*WID2+ij];
            ...
            {//compute filtered face values derivatives
               //compute h8 left face value
               fv_l = Realf(1.0/840.0) * (
                  (Realf)(- 3.0) * VALUEMASK(0)
                  + (Realf)(29.0) * VALUEMASK(1)
                  - (Realf)(139.0) * VALUEMASK(2)
                  + (Realf)(533.0) * VALUEMASK(3)
                  + (Realf)(533.0) * VALUEMASK(4)
                  - (Realf)(139.0) * VALUEMASK(5)
                  + (Realf)(29.0) * VALUEMASK(6)
                  - (Realf)(3.0) * VALUEMASK(7));
               fv_r = Realf(1.0/840.0) * (
                  - (Realf)(3.0) * VALUEMASK(1)
                  + (Realf)(29.0) * VALUEMASK(2)
                  - (Realf)(139.0) * VALUEMASK(3)
                  + (Realf)(533.0) * VALUEMASK(4)
                  + (Realf)(533.0) * VALUEMASK(5)
                  - (Realf)(139.0) * VALUEMASK(6)
                  + (Realf)(29.0) * VALUEMASK(7)
                  - (Realf)(3.0) * VALUEMASK(8));
               // compute h7 left face derivative
               fd_l = Realf(1.0/5040.0) * (
                  + (Realf)(9.0) * VALUEMASK(0)
                  - (Realf)(119.0) * VALUEMASK(1)
                  + (Realf)(889.0) * VALUEMASK(2)
                  - (Realf)(7175.0) * VALUEMASK(3)
                  + (Realf)(7175.0) * VALUEMASK(4)
                  - (Realf)(889.0) * VALUEMASK(5)
                  + (Realf)(119.0) * VALUEMASK(6)
                  - (Realf)(9.0) * VALUEMASK(7));
               fd_r = Realf(1.0/5040.0) * (
                  + (Realf)(9.0) * VALUEMASK(1)
                  - (Realf)(119.0) * VALUEMASK(2)
                  + (Realf)(889.0) * VALUEMASK(3)
                  - (Realf)(7175.0) * VALUEMASK(4)
                  + (Realf)(7175.0) * VALUEMASK(5)
                  - (Realf)(889.0) * VALUEMASK(6)
                  + (Realf)(119.0) * VALUEMASK(7)
                  - (Realf)(9.0) * VALUEMASK(8));

               Realf slope_abs,slope_sign;
               // scale values closer to 1 for more accurate slope limiter calculation
               const Realf scale = 1./minValue[wid3Index];
               slope_limiter(VALUEMASK(3)*scale, VALUEMASK(4)*scale, VALUEMASK(5)*scale, slope_abs, slope_sign);
               slope_abs = slope_abs*minValue[wid3Index];
               //check for extrema, flatten if it is
               bool is_extrema = (slope_abs == (Realf)(0.0));
               if (is_extrema) {
                  fv_r = (is_extrema) ? VALUEMASK(4) : fv_r;
                  fv_l = (is_extrema) ? VALUEMASK(4) : fv_l;
               ...
               bool filter = (VALUEMASK(3) - fv_l) * (fv_l - VALUEMASK(4)) < 0 || slope_sign * fd_l < (Realf)(0.0);
               if (filter) {
                  //Go to linear (PLM) estimates if not ok (this is always ok!)
                  fv_l= (filter) ? VALUEMASK(4) - slope_sign * (Realf)(0.5) * slope_abs : fv_l;
                  fd_l= (filter) ? slope_sign * slope_abs : fd_l;
               }
               //Fix right face if needed; boundary value is not bounded or slope is not consistent
               filter = (VALUEMASK(5) - fv_r) * (fv_r - VALUEMASK(4)) < 0 || slope_sign * fd_r < (Realf)(0.0);
               if (filter) {
                  //Go to linear (PLM) estimates if not ok (this is always ok!)
                  fv_r= (filter) ? VALUEMASK(4) + slope_sign * (Realf)(0.5) * slope_abs : fv_r;
                  fd_r= (filter) ? slope_sign * slope_abs : fd_r;
               }
            }
            {//filter pqm monotonicity
               /*fixed values give to roots clearly outside [0,1], or nonexisting ones*/

               /*second derivative coefficients, eq 23 in white et al.*/
               const Realf b0 =   (Realf)(60.0) * VALUEMASK(4) - (Realf)(24.0) * fv_r - (Realf)(36.0) * fv_l + (Realf)(3.0) * (fd_r - (Realf)(3.0) * fd_l);
               const Realf b1 = (Realf)(-360.0) * VALUEMASK(4) + (Realf)(36.0) * fd_l - (Realf)(24.0) * fd_r + (Realf)(168.0) * fv_r + (Realf)(192.0) * fv_l;
               const Realf b2 =  (Realf)(360.0) * VALUEMASK(4) + (Realf)(30.0) * (fd_r - fd_l) - (Realf)(180.0) * (fv_l + fv_r);
               ...
               const Realf root1 = (b2 != 0) ? (-b1 + sqrt_val) / ((Realf)(2.0) * b2) : 0;
               const Realf root2 = (b2 != 0) ? (-b1 - sqrt_val) / ((Realf)(2.0) * b2) : 0;

               /*PLM slope, MC limiter*/
               const Realf plm_slope_l = (Realf)(2.0) * (VALUEMASK(4) - VALUEMASK(3));
               const Realf plm_slope_r = (Realf)(2.0) * (VALUEMASK(5) - VALUEMASK(4));
               ...
               if(fixInflexion) {
                  const Realf valuesa = VALUEMASK(4);
                  Realf fva_l = fv_l;
                  Realf fva_r = fv_r;
                  Realf fda_l = fd_l;
                  Realf fda_r = fd_r;
            ...
            a[0] = fv_l;
            a[1] = fd_l/(Realf)(2.0);
            a[2] =  (Realf)(10.0) * VALUEMASK(4) - (Realf)(4.0) * fv_r - (Realf)(6.0) * fv_l + (Realf)(0.5) * (fd_r - (Realf)(3.0) * fd_l);
            a[3] = (Realf)(-15.0) * VALUEMASK(4)  + (Realf)(1.5) * fd_l - fd_r + (Realf)(7.0) * fv_r + (Realf)(8.0) * fv_l;
            a[4] =   (Realf)(6.0) * VALUEMASK(4) +  (Realf)(0.5) * (fd_r - fd_l) - (Realf)(3.0) * (fv_l + fv_r);
         }
#endif
