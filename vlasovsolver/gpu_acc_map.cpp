...
__global__ void scan_probe(
   ...
   __syncthreads();
   ColumnOffsets* columnData = dev_columnOffsetData + parallelOffset;
...
__global__ void __launch_bounds__(VECL,4) acceleration_kernel(
   ...
   const uint parallelCell = blockIdx.y;
   const uint cellOffset = parallelCell + cumulativeOffset;
   const uint w_tid = threadIdx.z*blockDim.x*blockDim.y + threadIdx.y*blockDim.x + threadIdx.x;

   const Vec* __restrict__ gpu_blockDataOrdered = dev_blockDataOrdered[parallelCell];
   const ColumnOffsets* __restrict__ columnData = dev_columnOffsetData + parallelCell;
...
#pragma omp for schedule(static,1)
      for (size_t cellIndex = 0; cellIndex < nLaunchCells; cellIndex++) {
         ...
         cellOffset = (uint)(cellIndex+cumulativeOffset);

         vmesh = spatial_cell->get_velocity_mesh(popID);
         nBlocksBeforeAdjust = vmesh->size();
         ...
         */
         ...
         host_totalColumns = host_returnLID[cpuThreadID][0];
         host_totalColumnSets = host_returnLID[cpuThreadID][1];
         host_recapacitateVectors = host_returnLID[cpuThreadID][2]; // columnData vectors
         host_valuesSizeRequired = (nBlocksBeforeAdjust + 2*host_totalColumns) * WID3 / VECL;
