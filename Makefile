...
# GPU settings
USE_WARPACCESSORS=0 # Set to nonzero value in order to utilize hashinator warp accessors
USE_GPU=0
ifeq ($(USE_CUDA),1)
	USE_GPU=1
...
ifeq ($(USE_GPU),1)
# Turn on compilation for of GPU-version of spatial_cell and spatial_batch
...
spatial_batch.o: spatial_batch_gpu.cpp
	@echo [CC] $<
	$(SILENT)$(CMP) $(CXXFLAGS) ${MATHFLAGS} $(FLAGS) -c spatial_batch_gpu.cpp -o spatial_batch.o $(INC_BOOST) ${INC_DCCRG} ${INC_EIGEN} ${INC_ZOLTAN} ${INC_VECTORCLASS} ${INC_FSGRID}
else
...
spatial_batch.o: spatial_batch_cpu.cpp
	@echo [CC] $<
	$(SILENT)$(CMP) $(CXXFLAGS) ${MATHFLAGS} $(FLAGS) -c spatial_batch_cpu.cpp -o spatial_batch.o $(INC_BOOST) ${INC_DCCRG} ${INC_EIGEN} ${INC_ZOLTAN} ${INC_VECTORCLASS} ${INC_FSGRID}
endif
